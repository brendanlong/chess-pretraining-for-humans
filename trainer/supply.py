...
win-probability gap that rating means, how many learnable items sit within the
selection jitter of it, and how far from target the 30 items selection would
draw from actually sit. That last is measured on an untouched bank, so it is a
floor — `pick_item` skips what a user has already answered, so a thin band
erodes into a wide one as they work through it, which is why the count matters
even where the drift still looks small.

    uv run python -m trainer.supply [--db data/items.db] [--floor 1000]

`--gaps` prints supply against gap instead. That is the form a plan to fill a
hole has to be written in, because gap is what mining and labeling can steer
with (`--min-gap-wp` / `--max-gap-wp` on both).
"""
        ...
        rows.append(
            {
                "user": user,
                "target": target,
                "gap": _gap_for_difficulty(target),
...
def band_width_in_gap(gap: float) -> float:
    """How much gap one band spans at this gap — the units the two tables differ in.
    ...
    lo = _gap_for_difficulty(difficulty + SELECTION_JITTER)
    return _gap_for_difficulty(max(difficulty - SELECTION_JITTER, 1e-9)) - lo
...
def by_gap(conn, step: float, floor: int) -> list[dict]:
    ...
    and summing it is the size of the labeling job.
    """
    rows = []
    hi = conn.execute("SELECT MAX(gap_wp) FROM items").fetchone()[0] or 0.0
    lo = 0.0
    while lo < hi:
        count = conn.execute(
        ...
        need = floor * step / band_width_in_gap(lo + step / 2)
        rows.append({"lo": lo, "hi": lo + step, "items": count, "need": need})
        rows[-1]["short"] = max(0.0, need - count)
        lo = round(lo + step, 4)
    return rows
...
def main() -> None:
    ...
    ap.add_argument("--floor", type=int, default=1000, help="mark bands thinner than this")
