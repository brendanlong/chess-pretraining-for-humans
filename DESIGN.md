...
Icons and the social-card image are committed PNGs in `web/`, generated by
`scripts/generate_assets.py` screenshotting Chromium — so the art has a
source that can be edited and re-run, and serving it stays a static file
read. The three pages each carry their own copy of the card metadata, which
a test pins so the copies can't drift apart.
